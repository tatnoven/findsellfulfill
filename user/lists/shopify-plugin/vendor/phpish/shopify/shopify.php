<?php
	...
	/* function validateSignature($query, $shared_secret)
    {
       ...
       $signatureBin = mhash(MHASH_SHA256, $string, $shared_secret);
		//$signatureBin = hash('sha256', $string, $shared_secret);
		 //invalid Request! Request or redirect did not come from Shopify
        $signature = bin2hex($signatureBin);

        return $query['hmac'] == $signature;
    }
 */	function validateSignature($query_params, $shared_secret) {
